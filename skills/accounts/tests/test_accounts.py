...
SCRIPT = Path(__file__).resolve().parents[1] / "scripts" / "accounts.py"
spec = importlib.util.spec_from_file_location("twitter_news_accounts", SCRIPT)
...
class AccountsSkillTests(unittest.TestCase):
    ...
    def test_twitter_news_home_shares_session_and_isolates_accounts_profile(self):
        with tempfile.TemporaryDirectory() as tmp, patch.dict(
            scan.os.environ, {"TWITTER_NEWS_HOME": tmp}, clear=True
        ):
            paths = scan.app_paths()
            self.assertEqual(Path(tmp) / "accounts", paths.root)
            self.assertEqual(Path(tmp) / "x-session.sqlite", paths.session_db)
            self.assertEqual(Path(tmp) / "scan.lock", paths.scan_lock)
            self.assertEqual(Path(tmp) / "accounts" / "state.sqlite", paths.state_db)

    def test_legacy_runtime_migrates_into_shared_toolkit(self):
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            legacy = base / "crypto-news-desk"
            toolkit = base / "twitter-news"
            profile = toolkit / "accounts"
            output = legacy / "output"
            output.mkdir(parents=True)
            (legacy / "x-session.sqlite").write_bytes(b"session")
            (legacy / "config.json").write_text("{}")
            old_manifest = output / "registry-scans" / "run" / "registry-manifest.json"
            (output / "latest.json").write_text(json.dumps({"manifest": str(old_manifest)}))
            roots = scan.migrate_legacy_roots(toolkit, profile, [legacy])
            self.assertEqual((toolkit, profile), roots)
            self.assertEqual(b"session", (toolkit / "x-session.sqlite").read_bytes())
            self.assertTrue((profile / "config.json").exists())
            latest = json.loads((profile / "output" / "latest.json").read_text())
            expected = profile / "output" / "registry-scans" / "run" / "registry-manifest.json"
            self.assertEqual(str(expected), latest["manifest"])
